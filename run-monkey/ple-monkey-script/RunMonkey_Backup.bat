...
:startMonkeyTesting
	...
	set Thisday=%DATE:/=%
    ::adb shell "monkey --ignore-crashes --ignore-timeouts --ignore-security-exceptions --ignore-native-crashes --kill-process-after-error --pkg-blacklist-file /data/local/tmp/BlackList.txt --throttle 200 -v -v 500000 > /data/local/tmp/MKY_Log/mky_event_%Thisday:~0,8%.txt \&"
	adb shell "monkey --ignore-crashes --ignore-timeouts --ignore-security-exceptions --ignore-native-crashes --kill-process-after-error --pkg-blacklist-file /sdcard/BlackList.txt --throttle 200 -v -v 500000 > /data/local/tmp/MKY_Log/mky_event_%Thisday:~0,8%.txt \&"
goto:eof
