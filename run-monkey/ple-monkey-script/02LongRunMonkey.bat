...
:startMonkeyTesting
	...
	set Thisday=%DATE:/=%
  adb shell "monkey --ignore-crashes --ignore-timeouts --ignore-security-exceptions --ignore-native-crashes --kill-process-after-error --pkg-blacklist-file /data/local/tmp/BlackList.txt --throttle 1000 -v -v 5000000 > /data/local/tmp/MKY_LOG/mky_event.txt 2>/data/local/tmp/MKY_LOG/mky_error_%Thisday:~0,8%.txt \&"
	::adb shell "monkey --ignore-crashes --ignore-timeouts --ignore-security-exceptions --ignore-native-crashes --kill-process-after-error --pkg-blacklist-file /sdcard/BlackList.txt --throttle 1000 -v -v 5000000 > /data/local/tmp/MKY_LOG/mky_event.txt 2>/data/local/tmp/MKY_LOG/mky_error_%Thisday:~0,8%.txt \&""
goto:eof
